...
namespace Web
{
    public partial class Register1 : System.Web.UI.Page
    {
        ...
        public bool Get_Yanzheng()
        {
            ...
            {
                MessageBox.ShowBox(this.Page, AC.GetLanguage("Verification"), Library.Enums.ModalTypes.warning);//验证码不能为空
            ...
            {
                MessageBox.ShowBox(this.Page, AC.GetLanguage("VerificationError"), Library.Enums.ModalTypes.error);//验证码错误 
                return false;
            }
        ...
        protected void btnSubmit_Click(object sender, EventArgs e)
        {
            if (RegValidate())
            {
                ...
                lgk.Model.tb_user recommendInfo = AC.userBLL.GetModel(AC.GetUserID(this.txtRecommendCode.Value.Trim()));//推荐用户
                ...
                userInfo.IdenCode = txtIDNumber.Value;// "身份证號";
                ...
                long iUID = AC.GetUserID(txtUserCode.Value.Trim());
                if (iUID > 0)
                {
                    ScriptManager.RegisterStartupScript(this.Page, this.GetType(), "hideloading", "hideloading();", true);
                    MessageBox.ShowBox(this.Page, AC.GetLanguage("RegistrationFails"), Library.Enums.ModalTypes.error);//注册失败，该会员编号已存在，请重新注册!
                }
                else
                {

                    if (AC.userBLL.Add(userInfo) > 0)
                    {

                        lgk.Model.tb_user model = AC.userBLL.GetModel(AC.GetUserID(userInfo.UserCode));
                    ...
                    {
                        lgk.Model.tb_user model = AC.userBLL.GetModel(AC.GetUserID(userInfo.UserCode));
                        AC.userBLL.Delete(model.UserID);//删除该会员资料
        ...
        protected bool RegValidate()
        {
            ...
            {
                MessageBox.ShowBox(this.Page, AC.GetLanguage("PleaseNumber"), Library.Enums.ModalTypes.warning);//请输入会员编号
            ...
            if (AC.GetUserID(txtUserCode.Value.Trim()) > 0)
            {
                MessageBox.ShowBox(this.Page, AC.GetLanguage("Memberexists"), Library.Enums.ModalTypes.info);//该会员编号已存在,请重新输入!
                return false;
            }
            #endregion
            ...
            {
                MessageBox.ShowBox(this.Page, AC.GetLanguage("Nickname"), Library.Enums.ModalTypes.warning);//会员昵称不能为空
                return false;
            }
            ...
            {
                MessageBox.ShowBox(this.Page, AC.GetLanguage("PasswordISNull"), Library.Enums.ModalTypes.warning);//登录密码不能为空
                return false;
            }
            ...
            {
                MessageBox.ShowBox(this.Page, AC.GetLanguage("ConfirmPasswordISNull"), Library.Enums.ModalTypes.warning);//确认密码不能为空
                return false;
            }
            if (!txtPassword.Value.Trim().Equals(txtRegPassword.Value.Trim()))
            {
                MessageBox.ShowBox(this.Page, AC.GetLanguage("TwoPasswordMatch"), Library.Enums.ModalTypes.warning);//两次输入的登录密码不一致
                return false;
            }
            if (txtSecondPassword.Value.Trim() == "")
            {
                MessageBox.ShowBox(this.Page, AC.GetLanguage("SecondaryISNUll"), Library.Enums.ModalTypes.warning);//二级密码不能为空
                return false;
            }

            if (txtRegSecondPassword.Value.Trim() == "")
            {
                MessageBox.ShowBox(this.Page, AC.GetLanguage("secondaryPasswordISNull"), Library.Enums.ModalTypes.warning);//确认二级密码不能为空
                return false;
            }
            if (!txtSecondPassword.Value.Trim().Equals(txtRegSecondPassword.Value.Trim()))
            {
                MessageBox.ShowBox(this.Page, AC.GetLanguage("TwoSecondaryMatch"), Library.Enums.ModalTypes.warning);//两次输入的二级密码不一致
                return false;
            }
            #endregion
            ...
            {
                MessageBox.ShowBox(this.Page, AC.GetLanguage("ReferenceNumberIsnull"), Library.Enums.ModalTypes.warning);//推荐人编号不能为空
                return false;
            }
            else
            {
                recommendInfo = AC.userBLL.GetModel(AC.GetUserID(reName));//推薦用户
                if (recommendInfo == null)
                {
                    MessageBox.ShowBox(this.Page, AC.GetLanguage("featuredNotExist"), Library.Enums.ModalTypes.warning);//该推荐会员不存在
                    return false;
                }
                if (recommendInfo.IsOpend == 0)
                {
                    MessageBox.ShowBox(this.Page, AC.GetLanguage("MemberISNull"), Library.Enums.ModalTypes.warning);//该会员尚未开通，不能作为推荐会员
                    return false;
                }
            }
            #endregion
            ...
            {
                MessageBox.ShowBox(this.Page, ac.GetLanguage("PhoneMust"), Library.Enums.ModalTypes.error);//联系电话格式错误
                return false;
            }

            int userid = ac.GetUserIDbByPhone(strPhoneNum);
            if (userid > 0)
            {
                MessageBox.ShowBox(this.Page, ac.GetLanguage("PhoneRegExists"), Library.Enums.ModalTypes.info);//该手机号码已注册
                return false;
            }

            int ct = ac.GetPhoneNumber(strPhoneNum);
            //  int p_phoneNumer = getParamInt("RegisterPhoneNumber");
            if (ct >= 1)
            {
                MessageBox.ShowBox(this.Page, string.Format(ac.GetLanguage("RegisterPhoneNumber")), Library.Enums.ModalTypes.warning);//该手机号码已注册
                return false;
            }
        ...
        protected void btnCreateUser_Click(object sender, EventArgs e)
        {
            ...
            while (bFlag)
            {
                ...
                strUserCode = strCode;
                if (!AC.userBLL.Exists(strUserCode))
                {
                    bFlag = false;
                }
            }
        ...
        protected void btnValidate_Click(object sender, EventArgs e)
        {
            ...
            if (strUserCode == "")
            {
                MessageBox.ShowBox(this.Page, AC.GetLanguage("PleaseNmae"), Library.Enums.ModalTypes.warning);//请输入用户名
                return;
            }
            //if (!PageValidate.checkUserCode(txtUserCode.Value.Trim()))
            //{
            ...
            if (AC.userBLL.Exists(strUserCode))
            {
                MessageBox.ShowBox(this.Page, AC.GetLanguage("existsrNmae"), Library.Enums.ModalTypes.info);//该用户名已存在
                return;
            }
            else
            {
                MessageBox.ShowBox(this.Page, AC.GetLanguage("availableNmae"), Library.Enums.ModalTypes.info);//该用户名可用
                return;
            }
        }
